...
$mb_password    = isset($_POST['mb_password']) ? trim((string)$_POST['mb_password']) : '';
$mb_password_re = isset($_POST['mb_password_re']) ? trim((string)$_POST['mb_password_re']) : '';
$mb_name        = isset($_POST['mb_name']) ? trim((string)$_POST['mb_name']) : '';
$mb_nick        = $mb_name;
$mb_email       = isset($_POST['mb_email']) ? trim((string)$_POST['mb_email']) : '';
...
$mb_open        = isset($_POST['mb_open'])          ? trim((string)$_POST['mb_open']) : "0";
$mb_open_default = isset($_POST['mb_open_default']) ? trim((string)$_POST['mb_open_default']) : $mb_open;
$mb_nick_default = isset($_POST['mb_nick_default']) ? trim((string)$_POST['mb_nick_default']) : $mb_nick;
$old_email       = isset($_POST['old_email'])       ? trim((string)$_POST['old_email']) : '';
...
if ($w == '' || $w == 'u') {
    ...
    if ($w=='') {
        ...
        if($config['cf_cert_use'] && $config['cf_cert_req']) {
            $cert_no = isset($_POST['cert_no']) ? trim((string)$_POST['cert_no']) : '';
            if($cert_no != $_SESSION['ss_cert_no'] || !$_SESSION['ss_cert_no'])
...
if (!$config['cf_use_email_certify']){
    ...
    if ((isset($_POST['mb_id']) ? trim((string)$_POST['mb_id']) : '') != $mb_id)
