...
if (isset($mobile))  { // search part (검색 파트[구간])
    //$mobile = mysql_real_escape_string($mobile);
    $qstr .= '&mobile=' . urlencode($mobile);
	$sql_search .= " and (REPLACE( mobile, '-', '') = '$mobile' or mobile = '$mobile')";
}
...
//SELECT * from  member where REPLACE( m_tel, '-', '') ='$tel' 
?>
<!-- 게시판 목록 시작 { -->
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="edu_head01 tbl_wrap">
        <table>
        <caption><?php echo $name?> 수료자 현황</caption>
        <thead>
        <tr>
            <th scope="col" class="none01">번호</th>
            <th scope="col" class="none02">기관</th>
            <th scope="col">교육명</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
       ...
       ?>
        <tr>
            <td class="td_num none01"><?php echo $num?></td>
            <td class="td_board none02"><?php echo $row['corp']?></td>
            <td class="td_subject"><?php echo $row['subject'] ?></td>
            <td class="td_board none03"><?php echo $row['schedule']?></td>
    ...
    <div class="bo_fx">
        <ul class="btn_bo_user">
            <li><a href="./index.php?case=list&amp;id=<?php echo $member['mb_id']?><?php echo $qstr?>" class="btn_b01 btn">목록</a></li>
        </ul>
    </div>
    </form>
</div>
