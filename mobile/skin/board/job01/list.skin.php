...
<div id="bo_list">
    ...
    <div class="btn_confirm write_div">
        <a href="./board.php?bo_table=<?php echo $bo_table ?>" class="<?php echo ($sfl == '')?"btn_b02":"btn_b01";?> btn">전체</a>
        <a href="./board.php?bo_table=<?php echo $bo_table ?>&sca=<?php echo $sca?>&sop=and&sfl=wr_10&stx=<?php echo urlencode("채용중")?>" class="<?php echo ($stx == '채용중')?"btn_b02":"btn_b01";?> btn">채용중</a>
        <a href="./board.php?bo_table=<?php echo $bo_table ?>&sca=<?php echo $sca?>&sop=and&sfl=wr_10&stx=<?php echo urlencode("채용완료")?>" class="<?php echo ($stx == '채용완료')?"btn_b02":"btn_b01";?> btn">채용완료</a>
    </div>
    ...
    <div class="list_01">
		...
		<?php if($list[$i]['wr_10'] == "채용완료"){ 
		echo "[<span style='color:blue;'>{$list[$i]['wr_10']}</span>] ";
		}else if($list[$i]['wr_10'] == "알선"){
		echo "[<span>{$list[$i]['wr_10']}</span>] ";	
		}else{
		echo "[<span style='color:red;'>채용중</span>] ";
		}?>   
                    <?php
                    if ($is_category && $list[$i]['ca_name']) {
                    ?>
                    <!--<a href="<?php echo $list[$i]['ca_name_href'] ?>" class="bo_cate_link">--><span class="bo_cate_link"><?php echo $list[$i]['ca_name'] ?></span><!--</a>-->
                    <?php } ?>
                    <span style="float:right; font-weight:400;">
                    <span class="sound_only">조회</span><!--<i class="fa fa-eye" aria-hidden="true"></i>-->조회수 : <?php echo number_format($list[$i]['wr_hit']) ?>회</span>
                    
                </div>
                <div class="bo_info">
<span class="sound_only">작성일</span><!--<i class="fa fa-clock-o" aria-hidden="true"></i>-->등록일 : <?php echo date("y-m-d", strtotime($list[$i]['wr_datetime'])) ?>
                    <span style="float:right"> 
&nbsp; 
채용마감 : 
<?php 
$wr7_date = isset($wr7[$i][0]) ? $wr7[$i][0] : '';
$wr7_type = isset($wr7[$i][1]) ? $wr7[$i][1] : '';
if($wr7_type == '날짜입력'){
	if($wr7_date && $today >= $wr7_date){
	 echo " 기간만료";
		}else{
	 echo $wr7_date ? date("y-m-d", strtotime($wr7_date)) : '';	
		}
}else{
	echo $wr7_type;
}
?>  
    ...
    <div class="bo_fx">
        <ul class="btn_bo_adm">
            ...
            <li><button type="submit" name="btn_submit" value="선택채용완료" onclick="document.pressed=this.value" class="btn"><i class="fa fa-files-o" aria-hidden="true"></i><span class="sound_only">선택채용완료</span></button></li>
...
<fieldset id="bo_sch">
    ...
    <select name="sfl" id="sfl">
    	<option value="wr_1"<?php echo get_selected($sfl, 'wr_1', true); ?>>기업명</option>
        <option value="wr_5"<?php echo get_selected($sfl, 'wr_5'); ?>>업종</option>
        <option value="wr_10"<?php echo get_selected($sfl, 'wr_10'); ?>>채용여부</option>
