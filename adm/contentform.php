...
if(!sql_query(" select gr_id from {$g5['content_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$g5['content_table']}`
                    ADD `gr_id` varchar(20) NOT NULL AFTER `co_id` ", true);						
}

if(!sql_query(" select co_rcontent from {$g5['content_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$g5['content_table']}`
                    ADD `co_rcontent` longtext NOT NULL AFTER `co_content` ", true);						
}
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        <th scope="row"><label for="gr_id">그룹ID</label></th>
        <td>
                <?php echo get_group_select('gr_id', $co['gr_id'], ''); ?><?php if ($w=='u') { ?><a href="javascript:document.location.href='./contentlist.php?sfl=a.gr_id&stx='+document.frmcontentform.gr_id.value;" class="btn_frmline">동일그룹 게시판목록</a><?php } ?>
        </td>
    </tr>    
    <tr>    
    ...
    <tr>
        <th scope="row">내용<br /><a href="./contentform.php?w=u&r=rb&co_id=<?php echo $co_id?>&sst=<?php echo $sst?>&sod=<?php echo $sod?>&sfl=<?php echo $sfl?>&stx=<?php echo $stx?>&page=<?php echo $page?>" style="color:#666">저장 복구</a></th>
        <td>
        <?php
		    	if($r == 'rb') $view = $co['co_rcontent']; 
    			else $view = $co['co_content'];
		?>		
		<?php echo editor_html('co_content', get_text($view, 0)); ?>
        </td>    
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            <select name="co_tag_filter_use" id="co_tag_filter_use">
                ...
                <option value="0"<?php echo get_selected($co['co_tag_filter_use'], 0); ?>>사용안함</option>            </select>
        </td>
    </tr>
    <tr>
