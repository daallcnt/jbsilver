    ...
    if ($rel_table || $rel_id || $rel_action)
    {
        ...
        $sql = " select * from {$g5['point_table']}
                    where mb_id = '$mb_id'
                      ...
                      and po_rel_action = '$rel_action' ";
        $row = sql_fetch($sql);
        $po_point = isset($row['po_point']) ? (int)$row['po_point'] : 0;
        $po_use_point = isset($row['po_use_point']) ? (int)$row['po_use_point'] : 0;
        $po_id = isset($row['po_id']) ? (int)$row['po_id'] : 0;

        if($po_point < 0) {
            $mb_id = $row['mb_id'];
            $po_point = abs($po_point);

            delete_use_point($mb_id, $po_point);
        } else {
            if($po_use_point > 0) {
                insert_use_point($row['mb_id'], $po_use_point, $po_id);
            }
        }
...
function sql_connect($host, $user, $pass, $db=G5_MYSQL_DB)
{
    ...
    if(function_exists('mysqli_connect') && G5_MYSQLI_USE) {
        // PHP 8.x default is throwing mysqli exceptions; legacy code expects FALSE on query failure.
