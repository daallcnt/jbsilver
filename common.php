...
function g5_path()
{
    ...
    if (!empty($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) !== 'off') {
        $is_https = true;
    } else if (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && stripos($_SERVER['HTTP_X_FORWARDED_PROTO'], 'https') !== false) {
        // reverse proxy(nginx) behind HTTP container
        $is_https = true;
    } else if (!empty($_SERVER['HTTP_X_FORWARDED_PORT']) && (string)$_SERVER['HTTP_X_FORWARDED_PORT'] === '443') {
        $is_https = true;
    }
...
if (!empty($_SESSION['ss_mb_id'])) { // 로그인중이라면
    ...
    if(!empty($member['mb_intercept_date']) && $member['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
        set_session('ss_mb_id', '');
        $member = array();
    } else {
        // 오늘 처음 로그인 이라면
        if (!empty($member['mb_id']) && substr(isset($member['mb_today_login']) ? $member['mb_today_login'] : '', 0, 10) != G5_TIME_YMD) {
            // 첫 로그인 포인트 지급
...
if ($bo_table) {
    ...
    if (is_array($board) && isset($board['bo_table']) && $board['bo_table']) {
...
if (G5_USE_MOBILE && $set_device) {
    $request_device = isset($_REQUEST['device']) ? $_REQUEST['device'] : '';
    if ($request_device == 'pc')
        $is_mobile = false;
    else if ($request_device == 'mobile')
        $is_mobile = true;
...
//------------------------------------------------------------------------------
if (G5_IS_MOBILE) {
    $bo_mobile_skin = isset($board['bo_mobile_skin']) ? $board['bo_mobile_skin'] : '';
    $board_skin_path    = get_skin_path('board', $bo_mobile_skin);
    $board_skin_url     = get_skin_url('board', $bo_mobile_skin);
    ...
    $bo_skin = isset($board['bo_skin']) ? $board['bo_skin'] : '';
    $board_skin_path    = get_skin_path('board', $bo_skin);
    $board_skin_url     = get_skin_url('board', $bo_skin);
