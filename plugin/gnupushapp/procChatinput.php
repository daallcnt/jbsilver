...
if($str_mp == $masterpassword)
{
	...
	if($is_member)
	{
		...
		if($_SESSION['reg_id'] && $gnu_config['chatting_admin_id'] == $mb_id && $gnu_config['chatting_admin'] == "D"){
			...
			$nick1 = substr($_SESSION['reg_id'], 50, 8);
			$nick_name = "user" . $nick1;
			$my_device = 1;
			$reg_id_anonymous = $_SESSION['reg_id'];
		}else{
	...
	if($is_member) $my_device = get_device_count_by_member_id($my_id);
	...
	if($my_device > 0 && $your_device > 0)
	{
		...
		else
		{
			...
			if($to_reg_id){
				...
				$etc = $use_profile . "ab&#ba" . $profile_link . "ab&#ba" . $pushstyle . "ab&#ba" . $image_src . "ab&#ba" . $ticker . "ab&#ba" . $bottom_text . "ab&#ba" . $type . "ab&#ba" . $banner . "ab&#ba" . $key . "ab&#ba" . $my_id . "ab&#ba" . $nick_name;
