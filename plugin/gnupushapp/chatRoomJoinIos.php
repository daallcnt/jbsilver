...
if($str_mp == $masterpassword && $reg_id && $room_id)
{
	if($my_id == "0") $my_id="";

	$row_tmp = sql_fetch(" select count(*) as 'cnt' from g5_gnupushapp_chatroom_ios where gpci_reg_id = '{$reg_id}' and cr_ix = '{$room_id}' ");
	...
	if($row_tmp['cnt'] == 0)
	{
		...
		$sql = " update g5_gnupushapp_chatroom_ios set gpci_lastdate = '".G5_TIME_YMDHIS."' where gpci_reg_id = '{$reg_id}' and cr_ix = '{$room_id}' ";
		sql_query($sql);
	}
