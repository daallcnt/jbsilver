...
switch($row['gp_type'])
{
	...
	case "quicksend" :
		...
		$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where gpr_mb_id in ({$mb_ids}) $sql_setting ";
	...
	case "new_product" :
		...
		if($data_array['m_page'] == 0)
		{
			...
			else
			{
				...
				if($ca_id)
				{
					$query = "select gss_reg_id,gss_mb_id,gss_sort,gss_other_setting,gss_sync from g5_gnupushapp_subscribe where gss_ca_name = '{$ca_id}' and gss_is_youngcart = 'Y' and gss_post_subscribe_onoff = 'Y' ";
					$devices_select_array = sql_query($query);
				}else{
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where gpr_setting_youngcart = 'Y' and gpr_setting_youngcart_all = 'Y' ";
					$devices_select_array = sql_query($query);
				}					
		...
		else
		{
			...
			if($ca_id)
			{
				$query = "select gss_reg_id,gss_mb_id,gss_sort,gss_other_setting from g5_gnupushapp_subscribe where gss_ca_name = '{$ca_id}' and gss_is_youngcart = 'Y' and gss_post_subscribe_onoff = 'Y'  order by gss_ix desc limit $limit, $push_div_num ";
				$devices_select_array = sql_query($query);
			}else{
				$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting from g5_gnupushapp_gcmregid where gpr_setting_youngcart = 'Y' and gpr_setting_youngcart_all = 'Y' order by gpr_ix desc limit $limit, $push_div_num ";
				$devices_select_array = sql_query($query);
			}
	...
	case "group" :
		...
		if($data_array['m_page'] == 0)
		{
			...
			else
			{
				...
				if($level == "all")
				{
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid WHERE 1 $sql_setting ";
				}
				elseif($level == "oldversion")
				{
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid WHERE 1 $sql_setting and gpr_version <> '{$gnu_config['webview_version']}'";
				}
				elseif($level == "1")
				{
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid WHERE 1 $sql_setting and gpr_sync = 'N' ";
				}
				elseif(is_numeric($level))
				{
					$sql_setting2 = str_replace('gpr_', 'a.gpr_', $sql_setting);
					$level = intval($level);
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid a join {$g5['member_table']} b on a.gpr_mb_id = b.mb_id and b.mb_level = {$level} where 1 $sql_setting2 ";
				}
				else
				{
					...
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where 1 $sql_setting and gpr_reg_id in ({$include_mb_id}) ";
				}
		...
		else
		{
			...
			if($level == "all")
			{
				$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid WHERE 1 $sql_setting $sql_limit";
			}
			elseif($level == "oldversion")
			{
				$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid WHERE 1 $sql_setting and gpr_version <> '{$gnu_config['webview_version']}' $sql_limit";
			}
			elseif(is_numeric($level))
			{
				$sql_setting2 = str_replace('gpr_', 'a.gpr_', $sql_setting);
				$level = intval($level);
				$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid a join {$g5['member_table']} b on a.gpr_mb_id = b.mb_id and b.mb_level = {$level} where 1 $sql_setting2 $sql_limit ";
			}
			else
			{
				$final_reg_id = array();
	...
	case "new_coupon" :
		...
		if($data_array['m_page'] == 0)
		{
			...
			if($devices_num > 0)
			{
				...
				{
					$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting from g5_gnupushapp_gcmregid a join {$g5['member_table']} b on a.gpr_mb_id = b.mb_id and b.mb_leave_date = '' and b.mb_intercept_date = '' and b.mb_id <> '{$data_array['cf_admin']}' where a.gpr_setting_youngcart = 'Y' ";
			...
			$query = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting from g5_gnupushapp_gcmregid a join {$g5['member_table']} b on a.gpr_mb_id = b.mb_id and b.mb_leave_date = '' and b.mb_intercept_date = '' and b.mb_id <> '{$data_array['cf_admin']}' where a.gpr_setting_youngcart = 'Y' $sql_limit";
			$devices_info = sql_query($query);
	...
	case "new_post" :
		...
		if($data_array['m_page'] == 0)
		{
			...
			elseif($func == "group")
			{
				...
				$query = "select gss_reg_id,gss_mb_id,gss_sort,gss_other_setting,gss_sync from g5_gnupushapp_subscribe where gss_is_youngcart = 'N' and gss_post_subscribe_onoff = 'Y' and gss_bo_table = '{$data_array['bo_table']}' and gss_post_subscribe = 'Y' and gss_reg_id in ({$include_mb_id}) ";
			...
			elseif($func == "reply")
			{
				...
				if($data_array['secret'] == 'secret')
				{
					...
					if($p_mb_id && $p_mb_id != $data_array['member_id'])
					{
						...
						$sql = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where gpr_setting_myreply = 'Y' and gpr_mb_id = '{$p_mb_id}' ";
		...
		if($data_array['m_page'] == 0)
		{
			...
			if($func == "normal")
			{
				...
				if($data_array['secret'] == 'secret')
				{
					if($p_mb_id && $p_mb_id != $data_array['member_id'])
					{
						...
						$sql = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where gpr_setting_mypost_com = 'Y' and gpr_mb_id = '{$p_mb_id}' ";
						$devices_a = sql_query($sql);
						for ($i=1; $rowddd=sql_fetch_array($devices_a); $i++)
						{
							if($rowddd['gpr_sort'] == 'I'){
					...
					else
					{
						...
						if(count($mbids) > 0)
						{
							...
							if($row_t['cnt'] > 0){
								...
								$sql = "select gpr_mb_id,gpr_reg_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where gpr_setting_mycom_tail = 'Y' and gpr_mb_id in ({$mb_ids}) ";
			...
			elseif($func == "comment_reply")
			{
				...
				if($data_array['secret'] == 'secret')
				{
					...
					{
						$sql = "select gpr_reg_id,gpr_mb_id,gpr_sort,gpr_other_setting,gpr_sync from g5_gnupushapp_gcmregid where gpr_setting_mycom_com = 'Y' and gpr_mb_id = '{$pc_mb_id}' ";
						$devices_a = sql_query($sql);
						for ($i=1; $rowddd=sql_fetch_array($devices_a); $i++)
						{
							if($rowddd['gpr_sort'] == 'I'){
...
if(!$returnEnd)
{
	...
	if(count($devices_select4) > 0 || count($newRegIdsIbs4) > 0 || count($newRegIdsIbm4) > 0 || count($newRegIdsIs4) > 0 || count($newRegIdsIm4) > 0)
	{
		$send4 = gnupushsend($devices_select4, $title4, $content, $address, $etc4, $newRegIdsIbs4, $newRegIdsIbm4, $newRegIdsIs4, $newRegIdsIm4, $newRegIdsId, $newRegIdsId, $gp_type, $pushchannel,array(),$keypass);
	}
	if(count($devices_select3) > 0 || count($newRegIdsIbs3) > 0 || count($newRegIdsIbm3) > 0 || count($newRegIdsIs3) > 0 || count($newRegIdsIm3) > 0)
	{
		$send3 = gnupushsend($devices_select3, $title3, $content, $address, $etc3, $newRegIdsIbs3, $newRegIdsIbm3, $newRegIdsIs3, $newRegIdsIm3, $newRegIdsId, $newRegIdsId, $gp_type, $pushchannel,array(),$keypass);
	}
	if(count($devices_select2) > 0 || count($newRegIdsIbs2) > 0 || count($newRegIdsIbm2) > 0 || count($newRegIdsIs2) > 0 || count($newRegIdsIm2) > 0)
	{
		$send2 = gnupushsend($devices_select2, $title2, $content, $address, $etc2, $newRegIdsIbs2, $newRegIdsIbm2, $newRegIdsIs2, $newRegIdsIm2, $newRegIdsId, $newRegIdsId, $gp_type, $pushchannel,array(),$keypass);
	}
	if(count($devices_select1) > 0 || count($newRegIdsIbs1) > 0 || count($newRegIdsIbm1) > 0 || count($newRegIdsIs1) > 0 || count($newRegIdsIm1) > 0 || count($devices_selectIphone) > 0)
	{
		$send1 = gnupushsend($devices_select1, $title1, $content, $address, $etc1, $newRegIdsIbs1, $newRegIdsIbm1, $newRegIdsIs1, $newRegIdsIm1, $devices_selectIphone, $newRegIdsId, $gp_type, $pushchannel,array(),$keypass);
	}

	if(count($devices_select4) >0 || count($newRegIdsIbs4) > 0 || count($newRegIdsIbm4) > 0 || count($newRegIdsIs4) > 0 || count($newRegIdsIm4) > 0 || count($devices_select3) >0 || count($newRegIdsIbs3) > 0 || count($newRegIdsIbm3) > 0 || count($newRegIdsIs3) > 0 || count($newRegIdsIm3) > 0 || count($devices_select2) >0 || count($newRegIdsIbs2) > 0 || count($newRegIdsIbm2) > 0 || count($newRegIdsIs2) > 0 || count($newRegIdsIm2) > 0 || count($devices_select1) >0 || count($newRegIdsIbs1) > 0 || count($newRegIdsIbm1) > 0 || count($newRegIdsIs1) > 0 || count($newRegIdsIm1) > 0 || count($devices_selectIphone) > 0){
