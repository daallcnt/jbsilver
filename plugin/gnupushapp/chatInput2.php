...
$in_is_member = "Y"; // 비회원이 관리자에게 문의한 경우에만 Y
$last_uid = "pushthumb";
$my_id = "";
...
if($str_mp == $masterpassword && $reg_id && $is_file == "Y" && $gnu_config['chatting_file'] != 'N')
{
	...
	if($row_tmp['cnt'] != 0)
	{
		$row_tmp2 = sql_fetch(" select * from g5_gnupushapp_chatroom_ios where gpci_random = '{$random}' and cr_ix = '{$room_id}' and gpci_reg_id = '{$reg_id}' ");
		if($row_tmp2['gpci_ix'])
		{
            if($issync == "Y"){
                $my_id = $row_tmp2['gpci_mb_id'];
				$my_member_info = get_member($my_id);
				$nick_name = $my_member_info['mb_nick'];
			}else{
                $room_join_data = sql_fetch(" select * from g5_gnupushapp_newchatting_room_joinlist where cr_ix = '{$room_id}' and reg_id <> '$reg_id' and (c_status = 'join' or c_status = 'nowjoin') ");
				if($gnu_config['chatting_admin_id'] == $$room_join_data['mb_id'] && $gnu_config['chatting_admin'] == "D"){
					$in_is_member = "N";
    ...
    else
    {
        ...
        // 방 정보 가져오기
        $room_info = sql_fetch("select * from g5_gnupushapp_newchatting_room where cr_ix = '$room_id'");
        ...
        if($in_is_member == "Y"){
            ...
            array_push($my_device_regid, $reg_id);
        }
        ...
        $member_list_from_room_id = sql_query("select * from g5_gnupushapp_newchatting_room_joinlist where cr_ix = '{$room_id}' and (c_status = 'join' or c_status = 'nowjoin') ");
        for ($i=0; $row_tmp=sql_fetch_array($member_list_from_room_id); $i++)
        {
            if($my_id != $row_tmp['mb_id'] && $reg_id != $row_tmp['reg_id'] && !in_array($row_tmp['reg_id'],$my_device_regid)){
        ...
        $etc = $use_profile . "ab&#ba" . $profile_link . "ab&#ba" . $pushstyle . "ab&#ba" . $image_src . "ab&#ba" . $ticker . "ab&#ba" . $bottom_text . "ab&#ba" . $type . "ab&#ba" . $banner . "ab&#ba" . $room_id . "ab&#ba" . $nick_name . "ab&#ba" . $room_type . "ab&#ba" . $last_uid;
        if(count($reg_ids) > 0){
