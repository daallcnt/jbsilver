...
if($is_member){
	...
	if (!$mb_id || !$mb_password){
		$error_m = '회원아이디나 비밀번호가 공백이면 안됩니다.';
		$error = true;
	}
	...
	if (!$mb['mb_id'] || !check_password($mb_password, $mb['mb_password'])) {
		$error_m = '가입된 회원아이디가 아니거나 비밀번호가 틀립니다.\\n비밀번호는 대소문자를 구분합니다.';
		$error = true;
	}
	...
	if ($mb['mb_intercept_date'] && $mb['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) {
		...
		$error_m = '회원님의 아이디는 접근이 금지되어 있습니다.\n처리일 : '.$date;
		$error = true;
	}
	...
	if ($mb['mb_leave_date'] && $mb['mb_leave_date'] <= date("Ymd", G5_SERVER_TIME)) {
		...
		$error_m = '탈퇴한 아이디이므로 접근하실 수 없습니다.\n탈퇴일 : '.$date;
		$error = true;
	}
	...
	<script>
		...
		window.myJs_err.callAndroid_diss();
	...
	if($error){
		...
		<script>
			...
			window.webkit.messageHandlers.callbackHandler.postMessage("error=<?php echo $error_m ?>");
		...
		window.myJs_err.callAndroid_err("<?php echo $error_m ?>");
...
if(preg_match("/GNUPUSH/", $_SERVER['HTTP_USER_AGENT']) && $reg_id = get_session('reg_id')) {

	$nick_name = $mb['mb_nick'];
	$member_id = $mb['mb_id'];
	sync_device_memb_id($member_id);
	$setting_default = "N%^N";
	$change_s = $setting_default;
...
window.webkit.messageHandlers.callbackHandler.postMessage("success=<?php echo $member_id; ?>");
...
window.myJs_reg.callAndroid_reg("<?php echo $member_id; ?>","<?php echo $nick_name; ?>","<?php echo $change_s; ?>");
